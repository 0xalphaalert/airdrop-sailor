...
import SubscriptionPageMobile from './mobile/pages/SubscriptionPageMobile';
import { useAppKit, useAppKitAccount } from '@reown/appkit/react';
import { useWriteContract, usePublicClient } from 'wagmi';
import { parseUnits } from 'viem';

const MERCHANT_WALLET = '0x7C558Fdd619D1D42A9427dA6e78c706dC74BF6e0';
...
const handleSubscriptionClick = async (plan) => {
  ...
  try {
    // Sepolia test payment configuration
    const paymentConfig = {
  "1_month": {
    amount: "2",
    tier: "Explorer Pass",
  ...
  "3_months": {
    ...
    const selectedPlan = paymentConfig[plan];

    if (!selectedPlan) {
      throw new Error("Invalid subscription plan");
    }

    console.log("Starting Base USDC payment:", {
  wallet: address,
  plan,
...
const amountInUSDC = parseUnits(selectedPlan.amount, 6);

// Send USDC directly to the merchant wallet
const txHash = await writeContractAsync({
  ...
  args: [
    ...
    // 2. Save payment to Supabase as pending
...
amount: selectedPlan.amount,
    ...
    if (paymentError) {
      ...
      throw new Error(
        "Transaction was submitted, but payment could not be recorded. Please contact support with your transaction hash."
      );
    }
...
// ==========================================
// 3. WAIT FOR BASE BLOCKCHAIN CONFIRMATION
// ==========================================
...
const { data: verificationData, error: verificationError } =
  await supabase.functions.invoke("verify-crypto-payment", {
    body: {
      tx_hash: txHash
    }
  });
...
if (verificationError) {
  ...
  setPaymentModal({
    ...
    message:
      'Your payment was sent successfully and is being verified on the blockchain.',
    tier: '',
    days: 0
  });
...
if (verificationData?.success) {
  ...
  setPaymentModal({
    ...
    message: 'Your Sailor Pass has been successfully activated.',
  ...
  setPaymentModal({
    ...
    message: 'Your transaction is waiting for blockchain verification.',
    tier: '',
    days: 0
  });
  ...
  if (isMobile) {
    return (
      <SubscriptionPageMobile 
        ...
        handleCheckout={handleSubscriptionClick}
  ...
  return (
    <div className="px-8 py-8 bg-[#F8FAFC] min-h-screen">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-[1000px] mx-auto">
            ...
            >
              ...
              {isPassActive ? (
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); handleSubscriptionClick("1_month"); }}
                ...
                >
                  {isProcessing ? 'Processing Payment...' : 'Get Explorer Pass'}
            ...
            >
              ...
              {isPassActive ? (
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); handleSubscriptionClick("3_months"); }}
                  disabled={isProcessing}
      ...
      {/* PAYMENT SUCCESS / STATUS MODAL */}
      {paymentModal.show && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md">

          <div className="relative w-full max-w-md overflow-hidden rounded-[28px] bg-white shadow-2xl animate-[paymentModalIn_0.35s_ease]">
            ...
            <button
              ...
              className="absolute top-4 right-4 w-9 h-9 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-900 text-xl transition-all"
            >
              ×
            </button>
            ...
            <div className="p-8 sm:p-10 text-center">
              ...
              {/* Message */}
              <p className="text-sm sm:text-base text-slate-500 leading-relaxed max-w-sm mx-auto mb-7">
                {paymentModal.message}
              </p>

              {/* PASS ACTIVATED CARD */}
              {paymentModal.type === 'success' && (
                <div className="rounded-2xl border border-indigo-100 bg-gradient-to-br from-indigo-50 to-blue-50 p-5 mb-7">

                  <div className="text-[10px] font-black tracking-[0.18em] text-indigo-500 mb-2">
                    ⚓ SAILOR PASS ACTIVATED
                  </div>
                  ...
                  <div className="text-sm text-slate-500 mt-1">
                    Active for {paymentModal.days} days
                  </div>
              ...
              <button
                ...
                className="w-full py-4 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold transition-all hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-indigo-200"
              >
                Continue Sailing ⚓
              </button>
