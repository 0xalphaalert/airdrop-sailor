...
import {
  ...
  Hexagon, Layers, Box, Sun, Star, Check,
  Flame, X, Upload, CalendarCheck, Twitter, MessageCircle, FileText, User,
} from "lucide-react";
  ...
  const fetchPageData = async () => {
    try {
      ...
      if (user) {
        ...
        // 2. Fetch Ledger to find when they last did tasks
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-8">
      <div className="max-w-[1400px] mx-auto">
        ...
        {/* CPX Research Offerwall Section (Replaced Featured Campaigns) */}
        <div className="mb-8">
          <div className="flex items-end justify-between mb-4">
            <div>
              <h2 className="text-xl font-black text-slate-900 tracking-tight">Earn More SAIL (Offerwall)</h2>
              <p className="text-xs font-medium text-slate-500 mt-1">Complete surveys and offers below to instantly grow your SAIL balance.</p>
            </div>
          </div>

          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm overflow-hidden min-h-[650px] flex flex-col">
            {user ? (
  <iframe 
  width="100%" 
  frameBorder="0" 
  height="2000px"  
  src={`https://offers.cpx-research.com/index.php?app_id=35858&ext_user_id=${user?.id || ''}&username=${encodeURIComponent(user?.user_metadata?.username || (typeof user?.email === 'string' ? user.email.split('@')[0] : 'User'))}&email=${encodeURIComponent(user?.email || '')}&subid_1=&subid_2=`}
  title="CPX Research Offerwall"
/>
            ) : (
              <div className="flex flex-col items-center justify-center py-24 text-center">
                <p className="text-slate-600 font-bold mb-4">Please sign in to access the offerwall and earn SAIL.</p>
                <button onClick={login} className="px-6 py-3 bg-blue-600 text-white font-bold rounded-xl shadow-md">Sign In / Register</button>
              </div>
            )}
          </div>
        </div>
