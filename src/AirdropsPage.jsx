...
// ============================================================================
const HeroDashboard = ({ topProject, stats }) => {
  return (
      ...
      <div className="w-full lg:w-[480px] bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 rounded-3xl p-6 lg:p-7 text-white relative overflow-hidden z-10 shrink-0 shadow-xl shadow-indigo-900/10">
        ...
        <div className="flex justify-between items-start mb-8">
          <div className="flex items-center gap-4">
            <img src={topProject?.logo_url || 'https://via.placeholder.com/50'} className="w-14 h-14 rounded-xl object-cover border border-white/10 bg-white" alt="Project Logo" />
            <div>
              <h2 className="text-xl font-black text-white tracking-tight">{topProject?.name || 'Loading...'}</h2>
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Tier {topProject?.tier || '-'}</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-y-4 gap-x-6 text-sm mb-6">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="font-bold text-white">{topProject?.task_count || 0} Tasks</span>
          </div>
        </div>
...
export default function AirdropsPage() {
  ...
  // 1. Replaced the manual resize block with the clean custom hook
  ...
  // 2. Updated to wait for user?.id specifically
  useEffect(() => {
    if (!user?.id) return;
    fetchProjects();
    fetchDashboardStats();
  ...
  useEffect(() => {
    if (!scoredProjects?.length) return;

    const topTen = scoredProjects
      .sort((a, b) => b._score - a._score)
      .slice(0, 10);

    const random =
      topTen[Math.floor(Math.random() * topTen.length)];

    setHeroProject(random);
  }, [scoredProjects]);
  ...
  const fetchProjects = async () => {
    try {
      ...
      // 3. Updated state setter to use standard previous merging
      setDashboardStats(prev => ({
        ...prev,
        avgTimeRequired: avgTime
      }));
  ...
  const fetchDashboardStats = async () => {
    try {
      const [
        ...
        supabase.from("tasks").select("cost")
      ]);

      const totalFarmCost = (taskCosts.data || []).reduce(
        (sum, task) => sum + (Number(task.cost) || 0),
        0
      );

      let currentSailBalance = 0;
      
      // 4. Added proper error handling for user profile fetch
      if (user?.id) {
        ...
        if (profileError) {
          ...
          currentSailBalance = Number(profile.lifetime_xp || 0);
        }
      }
      ...
      setDashboardStats(prev => ({
        ...
        totalFarmCost,
        sailBalance: currentSailBalance
      }));
  ...
  if (isMobile) {
    return (
      <>
        {/* 🚀 SEO FOR MOBILE VIEW */}
        <SEO 
          title="Airdrop Dashboard & Radar" 
          description="Track, farm and earn from the best airdrop opportunities. Access our AI narrative radar, whale activity, and live market sentiment." 
        />
        <AirdropsPageMobile
          loading={loading}
          projects={filteredProjects}
          topProject={heroProject}
          ...
          setFilterFunding={setFilterFunding}
  ...
  return (
    <div className="w-full h-full pb-20 font-sans text-slate-900 relative">
      ...
      <div className="w-full max-w-[1400px] mx-auto px-4 lg:px-8 pt-0 md:pt-2">
        ...
        <h2 className="text-xl font-black text-slate-900 tracking-tight mb-5">Airdrop Lists</h2>
