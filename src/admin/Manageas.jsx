...
Ensure there are a maximum of 5 competitor objects inside the "competitors" array. If some metrics are unknown, use rough market estimates or historical distribution tracking values for average_airdrop_usd. For the domain, provide the exact root website URL without https (e.g., spectral.finance).`;
    ...
    try {
      ...
      if (activeTab === 'projects') {
        ...
        const taskData = {
          ...
          sail_reward: parseInt(formData.sail_reward) || 0,
  ...
  <input
    type="number"
    min="0"
    value={formData.sail_reward ?? ''}
    onChange={(e) => handleInputChange('sail_reward', e.target.value)}
    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-blue-500 text-sm text-slate-900"
    placeholder="e.g. 100"
  />
</div>
                        <div>
