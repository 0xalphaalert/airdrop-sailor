...
import { Plus, Check, X, ExternalLink, Image as ImageIcon, Link as LinkIcon, Power, PowerOff, ToggleLeft, ToggleRight, Loader2, Save, Calendar, Edit2 } from 'lucide-react';
      ...
      <div className="bg-slate-50 border-b border-slate-200 px-6 pt-6">
        ...
        <div className="flex gap-6 text-sm font-bold overflow-x-auto scrollbar-hide">
          {[
            ...
            { id: 'platformTasks', label: 'Platform Tasks' },
      ...
      <div className="p-6">
        {loading ? <div className="text-slate-400 font-bold animate-pulse">Loading database records...</div> : (
          <>
            ...
            {/* --- TAB 5: PLATFORM TASKS --- */}
            {activeTab === 'platformTasks' && (
              <div>
                <div className="mb-8">
                  <div className="flex items-center justify-between gap-4 mb-4">
                    <div>
                      <h2 className="text-xl font-black text-slate-900 tracking-tight mb-1">Platform Tasks & Quests</h2>
                      <p className="text-sm font-medium text-slate-500">Create and manage quests for the Points Arena</p>
                    </div>
                    <button
                      onClick={() => {
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white text-sm font-bold rounded-lg transition-all shrink-0"
                    >
                ...
                {showForm && (
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 mb-8">
                    <h3 className="text-lg font-black text-slate-900 mb-6">{editingTaskId ? 'Edit Task' : 'Create New Task'}</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-600 mb-2 uppercase tracking-wider">XP Reward *</label>
                        <input
                          type="number"
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-600 mb-2 uppercase tracking-wider">Target URL (Optional)</label>
                        <input
                          type="url"
                      ...
                      <div className="flex items-center gap-3 pt-6">
                        ...
                        />
                        <label htmlFor="requiresProof" className="text-sm font-medium text-slate-700">User must submit a proof URL</label>
                      </div>
                      ...
                      <div className="flex items-center gap-3 pt-6">
                        <input
                          ...
                          className="w-5 h-5 rounded border-slate-300"
                        />
                        <label htmlFor="isPromoted" className="text-sm font-medium text-slate-700">Promoted Quest (Featured)</label>
                      </div>

                      {formData.is_promoted && (
                        <div>
                          <label className="block text-xs font-bold text-slate-600 mb-2 uppercase tracking-wider">Bonus XP</label>
                          <input
                            type="number"
                      ...
                      <div className="flex items-center gap-3 pt-6">
                        <input
                          ...
                          className="w-5 h-5 rounded border-slate-300"
                        />
                        <label htmlFor="isRecurring" className="text-sm font-medium text-slate-700">Recurring Quest</label>
                      </div>

                      {formData.is_recurring && (
                        <div>
                          <label className="block text-xs font-bold text-slate-600 mb-2 uppercase tracking-wider">Cooldown (Hours)</label>
                          <input
                            type="number"
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-600 mb-2 uppercase tracking-wider">Start Date</label>
                        <input
                          type="datetime-local"
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-600 mb-2 uppercase tracking-wider">End Date</label>
                        <input
                          type="datetime-local"
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="bg-slate-50 border-b border-slate-200">
                            <th className="text-left px-6 py-4 text-xs font-black text-slate-400 tracking-widest uppercase">Task Title</th>
                            <th className="text-left px-6 py-4 text-xs font-black text-slate-400 tracking-widest uppercase">Reward</th>
                            <th className="text-left px-6 py-4 text-xs font-black text-slate-400 tracking-widest uppercase">Target URL</th>
                            <th className="text-left px-6 py-4 text-xs font-black text-slate-400 tracking-widest uppercase">Proof Required</th>
                            <th className="text-left px-6 py-4 text-xs font-black text-slate-400 tracking-widest uppercase">Dates</th>
                            <th className="text-right px-6 py-4 text-xs font-black text-slate-400 tracking-widest uppercase">Status</th>
                          </tr>
                        </thead>
                        <tbody>
                          {tasks.map((task) => (
                            <tr key={task.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                              <td className="px-6 py-4">
                                <div>
                                  ...
                                  {task.is_promoted && <span className="ml-2 inline-block px-2 py-0.5 bg-amber-100 text-amber-700 text-[10px] font-bold rounded uppercase">Promoted</span>}
                                  {task.is_recurring && <span className="ml-2 inline-block px-2 py-0.5 bg-blue-100 text-blue-700 text-[10px] font-bold rounded uppercase">Recurring</span>}
                                </div>
                                {task.bonus_xp > 0 && <p className="text-xs font-medium text-slate-500 mt-1">+{task.bonus_xp} bonus XP</p>}
                              </td>
                              <td className="px-6 py-4"><span className="text-sm font-bold text-slate-900">+{task.xp_reward} XP</span></td>
                              <td className="px-6 py-4">
                                {task.target_url ? (
                                  <a href={task.target_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-sm font-medium text-blue-600 hover:text-blue-700 transition-colors">
                              ...
                              <td className="px-6 py-4">
                                <span className={`inline-block px-3 py-1 text-xs font-bold rounded-lg ${task.requires_proof ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                                  {task.requires_proof ? 'Yes' : 'No'}
                                </span>
                              </td>
                              <td className="px-6 py-4">
                                <div className="text-xs font-medium text-slate-600">
                                  {task.start_date && <div className="flex items-center gap-1"><Calendar className="w-3 h-3" /><span>Start: {new Date(task.start_date).toLocaleDateString()}</span></div>}
                                  {task.end_date && <div className="flex items-center gap-1 mt-1"><Calendar className="w-3 h-3" /><span>End: {new Date(task.end_date).toLocaleDateString()}</span></div>}
                                  {!task.start_date && !task.end_date && <span className="text-slate-400">No dates set</span>}
                                </div>
                              </td>
                              <td className="px-6 py-4">
                                <div className="flex items-center justify-end gap-2">
                                  <button onClick={() => handleEditTask(task)} className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold bg-blue-100 text-blue-700 hover:bg-blue-200 transition-all">
                                    <Edit2 className="w-4 h-4" /> Edit
                                  </button>
                                  <button
                                    onClick={() => handleToggleActive(task.id, task.is_active)}
                                    className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all ${task.is_active ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
                                  >
                                    {task.is_active ? <><ToggleRight className="w-4 h-4" /> Active</> : <><ToggleLeft className="w-4 h-4" /> Inactive</>}
                                  </button>
                                </div>
                              </td>
                            </tr>
