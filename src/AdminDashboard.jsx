...
export default function AdminDashboard() {
  ...
  // 1. PASTE THE HELPER FUNCTION HERE (Inside the component, right above handleAutoFillX)
  const autoMigrateLogoToImgBB = async (xUrl, entityName) => {
    ...
    try {
      const proxyTarget = `https://unavatar.io/twitter/${handle}?fallback=false`;
      const imageResponse = await fetch(proxyTarget);
      if (!imageResponse.ok) throw new Error(`Status: ${imageResponse.status}`);
      const imageBlob = await imageResponse.blob();

      const payload = new FormData();
      payload.append('image', imageBlob, `${handle}_logo.png`);
      
      const IMGBB_API_KEY = '1de173c5b97e6a61196a6f5153b93960'; 
      const uploadRequest = await fetch(`https://api.imgbb.com/1/upload?key=${IMGBB_API_KEY}`, { 
        method: 'POST', 
        body: payload 
      });
      ...
      console.error(`Asset migration engine pipeline failure for ${entityName}:`, error);
    }
    return null;
  };

  // 2. REPLACE YOUR EXISTING handleAutoFillX WITH THIS
  const handleAutoFillX = async () => {
    ...
    try {
      ...
      const permanentCdnLink = await autoMigrateLogoToImgBB(formData.x_link, handle);
      ...
      const { data: fundData, error: fundError } = await supabase
        ...
        .select('funding_amount, lead_investor')
      ...
      setFormData(prev => ({
        ...prev,
        name: cleanName,
        logo_url: permanentCdnLink || prev.logo_url || `https://api.dicebear.com/7.x/shapes/svg?seed=${cleanName}`,
      ...
      alert("X scan completed with safe canvas fallbacks!");
    } finally { setIsFetchingX(false); }
  };

  // 3. REPLACE YOUR EXISTING handleSaveFunding WITH THIS
  const handleSaveFunding = async (e) => {
    ...
    if (fundingForm.x_link) {
      const permanentCdnLink = await autoMigrateLogoToImgBB(fundingForm.x_link, fundingForm.project_name);
      if (permanentCdnLink) {
        finalLogoUrl = permanentCdnLink;
      }
    }
    ...
    const payload = {
      ...
      project_logo: finalLogoUrl || `https://api.dicebear.com/7.x/shapes/svg?seed=${fundingForm.project_name}`,
