...
import { 
  ...
  Sparkles, X, Copy, Check, Loader2 // 🚀 NEW ICONS
} from 'lucide-react';
...
// --- 🚀 THE TRUE BROWSERLESS EXPORT ---
  ...
  return (
    <div className="min-h-[100dvh] bg-slate-900 flex flex-col lg:flex-row text-slate-200 font-sans overflow-x-hidden overflow-y-auto lg:overflow-hidden">
      
      {/* 🛠️ THE NEW CASCADING SIDEBAR */}
<div className="w-full lg:w-[340px] bg-slate-800 flex flex-col h-auto lg:h-screen flex-shrink-0 z-20 shadow-xl border-b lg:border-b-0 lg:border-r border-slate-700">
...
<div className="flex-1 overflow-y-auto space-y-1.5 pr-1 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent min-h-[300px] lg:min-h-0">
      ...
      {/* ======================================================== */}
      <div className="w-full flex-1 flex flex-col min-h-[450px] lg:h-screen relative bg-black pb-24 lg:pb-0">
        
        {/* Top Control Bar */}
        <div className="h-16 border-b border-slate-800 bg-slate-900 flex items-center justify-between px-4 lg:px-8 shrink-0">
        ...
        <div className="flex-1 overflow-hidden relative flex items-center justify-center bg-[#050505]">
          ...
          <div className="absolute transform scale-[0.28] sm:scale-[0.45] md:scale-[0.6] lg:scale-[0.75] xl:scale-[0.9] 2xl:scale-100 origin-center transition-transform duration-300">
...
<img src="https://pddykfluvauwsfleqsfk.supabase.co/storage/v1/object/public/assets/logo-icon.png" alt="" className="w-4 h-4 invert brightness-0" crossOrigin="anonymous" />
        ...
        {/* ======================================================== */}
        <div className="lg:hidden fixed bottom-0 left-0 right-0 p-4 bg-slate-900/90 backdrop-blur-md border-t border-slate-800 z-50 pb-safe flex gap-3">
          <button 
            onClick={handleGenerateTweet}
            className="flex-1 flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-4 py-4 rounded-2xl font-black text-sm transition-all shadow-sm border border-slate-700"
          >
            <Sparkles className="w-5 h-5 text-amber-400" /> Draft
          </button>
          <button 
            onClick={handleExport} 
            disabled={isCapturing}
            className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 disabled:text-slate-500 text-white px-4 py-4 rounded-2xl font-black text-sm transition-all shadow-[0_0_20px_rgba(37,99,235,0.3)]"
          >
            {isCapturing ? (
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative w-full max-w-2xl bg-slate-900 rounded-3xl shadow-2xl border border-slate-700 flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6">
              {isGeneratingTweet ? (
                ...
                <textarea 
                  ...
                  className="w-full h-64 bg-slate-950 border border-slate-800 rounded-xl p-4 text-slate-200 text-sm font-medium leading-relaxed focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 resize-none"
            ...
            <div className="px-6 py-5 border-t border-slate-800 bg-slate-800/30 flex justify-end gap-3">
              ...
              >
                {isCopied ? <><Check className="w-4 h-4" /> Copied!</> : <><Copy className="w-4 h-4" /> Copy to X</>}
              </button>
            </div>
          </div>
        </div>
